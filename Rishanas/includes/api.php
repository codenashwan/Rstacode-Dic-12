...
class db_object{
 protected static $table_name = "word"; 
    ...
    public static function get_all($condition) {
        if($condition === 0){
        return static::query_proccess("SELECT * FROM ".static::$table_name." ORDER BY `date` DESC ");
    }else{
        return static::query_proccess("SELECT * FROM ".static::$table_name." $condition ");
    }
}
    ...
    public static function count_rows($condition){
        global $db;
        if($condition === 0){
        return mysqli_num_rows($db->query("SELECT * FROM ".static::$table_name.""));
        }else{
        return mysqli_num_rows($db->query("SELECT * FROM ".static::$table_name." WHERE $condition "));
        }
    }
